...
#ifdef POOL_DEBUG
            hm_log(GCLOG_TRACE, pool->log, "{Pool}: old freenode: %p/%d, new freenode: %p/%d from pool:%p/%d", p->freenode, p->freenode->realsize, node, node->realsize, p, p->size);
#endif
            node->used = 0;
...
static void *pool_get_node(struct hm_pool_s *pool, const int realsize)
{
    ...
    if (node->next == NULL) {
#ifdef POOL_DEBUG
        hm_log(GCLOG_TRACE, pool->log, "no more freenodes remaining in pool: %p/%d, creating new bucket", pool, pool->size);
#endif
...
#ifdef POOL_DEBUG
    hm_log(GCLOG_TRACE, pool->log, "{Pool}: returning node: %p/%d, new freenode: %p from pool: %p/%d", node, realsize, pool->freenode, pool, pool->size);
#endif
...
#ifdef POOL_DEBUG
        hm_log(GCLOG_TRACE, pool->log, "new node: %p in pool: %p old freenode: %p pool size: %d node ptr: %p reg: %p", node, pool, pool->freenode, pool->size, node->ptr, *(void **)node->ptr);
...
void *hm_prealloc(struct hm_pool_s *pool, void *ptr, const int size)
{
    ...
    if (ptr == NULL && size == 0) {
#ifdef POOL_DEBUG
        hm_log(GCLOG_TRACE, pool->log, "{Pool}: doing nothing");
#endif
        /** do nothing */
        return NULL;
    } else if (ptr == NULL && size != 0) {
#ifdef POOL_DEBUG
        hm_log(GCLOG_TRACE, pool->log, "{Pool}: malloc() size: %d", size);
#endif
        /** malloc() */
        return hm_palloc(pool, size);
    } else if (ptr != NULL && size == 0) {
#ifdef POOL_DEBUG
        hm_log(GCLOG_TRACE, pool->log, "{Pool}: free() size: %d", size);
#endif
        /** free() */
...
#ifdef POOL_DEBUG
        hm_log(GCLOG_TRACE, pool->log, "{Pool}: status quo for size: %d", size);
#endif
        /** nothing needs to be changed - return exactly the same pointer */
        return ptr;
    } else if (ptr != NULL && size > 0 && node != NULL && node->size != size) {
#ifdef POOL_DEBUG
        hm_log(GCLOG_TRACE, pool->log, "{Pool}: realloc for old size: %d new size: %d", node->size, size);
#endif
        /** realloc() */
...
#ifdef POOL_DEBUG
            hm_log(GCLOG_TRACE, pool->log, "{Pool}: found existing pool: %p/%d", p, p->size);
#endif
...
#ifdef POOL_DEBUG
    hm_log(GCLOG_TRACE, pool->log, "{Pool}: creating new pool with parent pool: %p/%d", p, p->size);
#endif
